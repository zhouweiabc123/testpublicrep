...
class TestDouZhen:
    ...
    # 如果配置文件里的dev值为true的话，时间在09:00:00到18:00:00之间使用dev环境，gameId使用'73'，否则用线上环境
    ...
    def get_openId(self,get_age,new_tourist_data,mk_time):
        ...
        param_json = {
            ...
            'sign': sign,
            'indexId': indexId,
            "envKey": data[0]['game80']['envKey'],
            'tsKey': data[0]['game80']['tsKey'],
            'snKey': data[0]['game80']['snKey'],
            'zoneKey': data[0]['game80']['zoneKey']
        }
        js = {
    ...
    def order_login(self):
        ...
        with allure.step("发送请求"):
            res = requests.post(url=self.configurl + data[0]['api'], json={
                "account": data[0]["game80dev"]['user'],
                "password": data[0]["game80dev"]['password'],
                "gameId": self.gameId,
                "apothecary_inexcusable_willful": None,
                "fast_shelve": None,
                "rhyme_systolic": None
    ...
    def order_openId(self,order_login,mk_time,get_age):
        ...
        data = yaml.full_load(open(fr"{self.yml_path}/index.yml", "r", encoding="utf-8"))
        indexId = data[0]['game80dev']['indexId']
        ...
        param_json = {
            ...
            'sign': sign,
            'indexId': indexId,
            "envKey": data[0]['game80dev']['envKey'],
            'tsKey': data[0]['game80dev']['tsKey'],
            'snKey': data[0]['game80dev']['snKey'],
            'zoneKey': data[0]['game80dev']['zoneKey']
        }
        res = requests.post(url=self.index_url, json=param_json, headers=header)  # url=data[0]["api"]
        response = res.json()
        i = 1
    ...
    def test_14_creat_order(self, order_login, order_openId, mk_time, order_num, order_data):
        ...
        indexId=idexdata[0]['game80dev']['indexId']
        #orderdata=None
        '''斗争的魔气'''
        ...
        gameOrderNo=order_num+order_data['devgame80']['gameOrderNo']
        mylog.mylog.info(f"orderdata：{order_data}")
