...
@allure.epic("X项目幻刃录游戏")
@allure.feature("幻刃录登录注册模块")
class TestHuanRengLu:
    ...
    def get_openId(self,get_age,new_tourist_data,mk_time):
        ...
        param_json = {
            ...
            'sign': sign,
            'indexId': indexId,
            "envKey": data[0]['game79']['envKey'],
            'tsKey': data[0]['game79']['tsKey'],
            'snKey': data[0]['game79']['snKey'],
            'zoneKey': data[0]['game79']['zoneKey']
        }
        js = {
    ...
    def order_login(self):
        ...
        with allure.step("发送请求"):
            res = requests.post(url=self.configurl + data[0]['api'], json={
                "account": data[0]["game79dev"]['user'],
                "password": data[0]["game79dev"]['password'],
                "gameId": self.gameId,
                "apothecary_inexcusable_willful": None,
                "fast_shelve": None,
                "rhyme_systolic": None
    ...
    def order_openId(self,order_login,mk_time,get_age):
        ...
        data = yaml.full_load(open(fr"{self.yml_path}/index.yml", "r", encoding="utf-8"))
        indexId = data[0]['game79dev']['indexId']
        ...
        param_json = {
            ...
            'sign': sign,
            'indexId': indexId,
            "envKey": data[0]['game79dev']['envKey'],
            'tsKey': data[0]['game79dev']['tsKey'],
            'snKey': data[0]['game79dev']['snKey'],
            'zoneKey': data[0]['game79dev']['zoneKey']
        }
        res = requests.post(url=self.index_url, json=param_json, headers=header)  # url=data[0]["api"]
        response = res.json()
        i = 1
    ...
    def test_14_creat_order(self, order_login, order_openId, mk_time, order_num, order_data):
        ...
        indexId=idexdata[0]['game79dev']['indexId']
        #orderdata=None
        '''幻刃录'''
        ...
        gameOrderNo=order_num+order_data['devgame79']['gameOrderNo']
